...
exp_train_config = {
           ...
           "--test_data_type": "harvey",    # "geocorpora"
           "--train_data_dir": "./datasets",
           "--validate_data_dir": "./datasets",
           "--test_data_dir": "./datasets/HarveyTweet2017", # "./datasets/GeoCorpora"
           "--train_data_file": "train.txt",
           "--validate_data_file": "valid.txt",
           "--test_data_file": "GeoCorpora.txt",
           "--is_validate": 1,
           "--is_test": 1,
           "--output_dir": "./outputs",
           "--cache_dir": "./cache",
...
data_processer_map = {
    ...
    "geocorpora": Processor_Standard_V1
}
...
class TopoBertModelTrainer():
    ...
    def __init__(self, model, tokenizer = None, train_config = _default_train_config):
        ...
        if self.train_config['--test_data_type'] != "harvey":
    ...
    def train(self):
        ...
        # Get device:
        device = torch.device(f"cuda:{self.train_config['--use_gpu']}" if torch.cuda.is_available() else "cpu")
    ...
    def test(self, model_dir:str):
        # load model config
        current_model_config = json.load(open(os.path.join(model_dir, "model_config.json")))
        # init model & load pretrained parameters
        self.model = BertCNN1DNer.from_pretrained(model_dir, model_config=current_model_config)
        # get training config
        current_training_config = json.load(open(os.path.join(model_dir, 'train_config.json')))

        self.evaluate(use_strict=False)
...
def main():
    model_args_used = {
       "--cuda": f"cuda:{exp_train_config['--use_gpu']}",
    ...
    # Specify model:
    config = BertConfig.from_pretrained(model_args_used['--pretrained_model'], num_labels=model_args_used['--num_of_labels'], finetuning_task='geoparse')
    ...
    current_trainer.test(model_dir = './outputs/20230516170910')
