...
class BertCNN2DNer(BertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None, attention_mask_label=None):
        ...
        batch_size,max_len,feat_dim = sequence_output.shape
        valid_output = torch.zeros(batch_size,max_len,feat_dim,dtype=torch.float32,device=self.model_config['--cuda'])
        for i in range(batch_size):
            jj = -1
            for j in range(max_len):
                    if valid_ids[i][j].item() == 1:
...
class BertCNN1DNer(BertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None, attention_mask_label=None):
        ...
        valid_output = torch.zeros(batch_size, max_seq_len, hidden_layer_size, dtype=torch.float32, device=input_ids.device)
        for i in range(batch_size):
            jj = -1
